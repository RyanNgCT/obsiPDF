...
description: Format explicitly selected Obsidian-flavoured Markdown notes for polished PDF export by dynamically inserting managed LaTeX whitespace that prevents orphaned headings and split callout list items. Use when editing Markdown and validating an Obsidian PDF export as a source/PDF pair; do not use for unrelated PDF styling or files the user has not placed in scope.
author: "@RyanNgCT"
---
...
4. Inspect every PDF page, using both rendered page images and positioned text extraction when available. Check only these required invariants:
   ...
   - Each list item inside an Obsidian callout starts and ends on the same page.
5. Fix the earliest violation. Insert the smallest plausible managed spacer immediately before the heading or list item with `manage_spacers.py insert`. For a callout item, inherit its blockquote prefix so the spacer remains inside the callout.
...
- If one item is taller than a printable page, or the same target fails to improve after three spacing adjustments, stop adjusting that target and report it rather than creating a blank page or looping indefinitely.
...
`--lines N` writes `N` LaTeX line-break commands. `insert` automatically inherits a target line's `>` blockquote prefix; use `--quote-prefix` only when automatic detection is insufficient.
