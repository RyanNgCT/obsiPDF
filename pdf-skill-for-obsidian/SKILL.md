...
5. Analyze all pages programmatically and visually. Fix the earliest confirmed violation unless corrections are proven independent:
   - Every H1-H6 heading must share a page with the complete block it introduces. If the first block is a callout, or one short introductory paragraph leads directly to a callout, keep the heading, introduction, and complete callout together by spacing before the heading. If the first block is raster media followed by an explanatory list, keep the heading, media, and complete accompanying list together. Keep consecutive heading groups together by spacing before the earliest heading.
