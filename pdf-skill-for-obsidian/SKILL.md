...
9. Record the SHA-256 of the fully verified temporary PDF, then export the final PDF to the confirmed or derived path with the same settings. If that PDF already exists, replace it only now and accept the system overwrite-confirmation dialog. Compare the final PDF's SHA-256 with the verified temporary PDF: an exact match completes final verification without another render, analyzer run, or page-by-page inspection. If the hashes differ, keep the temporary reference and perform the complete final analyzer and visual-inspection pass. Delete the run-specific temporary PDFs and images outside the note folder only after verification, then report the final PDF path, spacer count, settings, and any unavoidable exceptions.
